...
public class CodeIndicatorsAnalyzer {
	...
	private void addValueToIndicator(IndicatorId indicator, int value) {
	...
	private void processClasses(Document doc) {
		...
		for (int i = 0; i < classes.size(); i++) {
			for (Document codesmell : (List<Document>) classes.get(i).get("codesmells")) {
				...
				if (indicator.equals(IndicatorId.COMPLEX_METHOD)
						|| indicator.equals(IndicatorId.BRAIN_METHOD)) {
	...
	public void detectBugs(long fileshash, String snapshot) {
		...
		addValueToIndicator(IndicatorId.AUTOMATIC_STATIC_ANALYSIS_ISSUES, bugs.size());
		...
		for (Document bug : bugs) {
			...
			if (category.equals("MT_CORRECTNESS")) {
				indicator = IndicatorId.MULTITHREAD_CORRECTNESS;
			} else if (category.equals("PERFORMANCE")) {
				indicator = IndicatorId.SLOW_ALGORITHM;
			}
